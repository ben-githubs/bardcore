...
class CompTrack(Playable):
    ...
    def play(self, track_name: str = None, vol: float = 1):
        ...
        """
        target_track = self.tracks[track_name] if track_name else list(self.tracks.values())[0]
        ...
        self.current_track = target_track or self.current_track or list(self.tracks.values())[0]
...
class TrackList(Playable):
    ...
    def play(self, track_name: str = None, vol: float = 1, loop: bool = True, shuffle: bool = True):
        kwargs = {
        ...
        if self.is_playing:
            if track_name != self.current_track.name:
                ...
                logging.warning("Switching tracks of a track list is not currently supported.")
            else:
                logging.info("Cannot start TrackList '{self.name}' because it is already playing")
            return
    ...
    def play_async(self, track_name: str = None, vol: int = 1, loop: bool = False, shuffle: bool = False):
        ...
        if track_name:
            ...
            if not track:
                logging.error(f"TrackList '{self.name}' has no track named '{track_name}'")
                self.thread = None
                return
    ...
    def stop(self, fade_dur: float = 2.5):
        ...
        if self.is_playing:
            ...
            self.stop_event.fade_dur = fade_dur # Feels like this is bad practice but meh
            self.stop_event.set()
